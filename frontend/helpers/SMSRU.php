...
use common\models\SmsRuNotification;
use Yii;
use stdClass;
...
class SMSRU
{
    ...
    {
        $apiKey = Yii::$app->keyStorage->get('sms.api.key', null);

        $smsLog = new SmsNotification([
            'phone' => $phone,
            'message' => $message,
        ]);
        ...
        if ($apiKey) {
            ...
            $data->from = 'PORTAL_SKR'; // Если у вас уже одобрен буквенный отправитель, его можно указать здесь, в противном случае будет использоваться ваш отправитель по умолчанию
            // $data->time = time() + 7*60*60; // Отложить отправку на 7 часов
            // $data->translit = 1; // Перевести все русские символы в латиницу (позволяет сэкономить на длине СМС)
            $data->test = YII_ENV_DEV; // Позволяет выполнить запрос в тестовом режиме без реальной отправки сообщения
            // $data->partner_id = '1'; // Можно указать ваш ID партнера, если вы интегрируете код в чужую систему
            $sms = $smsru->send_one($data); // Отправка сообщения и возврат данных в переменную

            $smsLog->code = $sms->status_code;
            $smsLog->response = $sms->status_text;

            $isSuccess = false;

            if ($sms->status == "OK") { // Запрос выполнен успешно
                $isSuccess = true;
                $smsLog->external_id = $sms->sms_id;
                $smsLog->balance = $sms->balance;
            }
